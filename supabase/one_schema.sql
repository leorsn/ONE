-- ONE canonical cloud item model.
-- Apply only to the dedicated ONE Supabase project.
-- Local-only state such as notification IDs, local attachment URIs and sync state
-- intentionally does not belong in this table.
...
create table if not exists public.items (
  ...
  type text not null check (
    type in ('task','reminder','appointment','event','note','link','idea','travel','shopping','document')
  ),
  item_date date,
  item_time time,
  ...
  source_type text not null check (
    source_type in ('manual','share','scan','email','screenshot','photo','link','system')
  ),
  source_app text,
  original_text text,
  attachment_url text,
