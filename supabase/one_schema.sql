...
create table if not exists public.items (
  ...
  title text not null,
  raw_input text,
  type text not null check (type in ('task','reminder','appointment','event','note','link','idea','travel','shopping')),
  item_date date,
  item_time time,
  ...
  source_type text not null check (source_type in ('manual','share','email','screenshot','photo','link','system')),
  source_app text,
  original_text text,
  attachment_url text,
  image_url text,
