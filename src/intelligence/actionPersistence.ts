...
export function persistExecutedAction(
  item: OneItem,
  record: IntelligenceActionRecord,
  now = new Date()
): PersistedActionOutcome {
  ...
  const executedActions: OneInboxAction[] = Array.from(
    new Set<OneInboxAction>([...(item.executedActions ?? []), record.action])
  );
...
export function hasExecutedAction(item: OneItem, action: OneInboxAction) {
  return Boolean(item.executedActions?.includes(action));
}
