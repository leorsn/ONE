...
export function TriageRow({
  ...
  return (
    <View style={[styles.row, { borderBottomColor: `${theme.text}0D` }]}>
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={`Open ${item.title}`}
        onPress={onOpen}
        style={({ pressed }) => [styles.openArea, { backgroundColor: pressed ? `${theme.fill}42` : 'transparent' }]}
      >
      ...
      {action ? (
        ...
        >
          <Text style={[styles.actionText, { color: action.primary ? theme.sky : theme.textSecondary }]}>{action.label}</Text>
          {action.primary ? <OneIcon name={icons.chevron} size={11} color={theme.sky} /> : null}
        </Pressable>
      ) : null}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  openArea: { minHeight: 94, paddingHorizontal: 16, paddingVertical: 13, flexDirection: 'row', alignItems: 'center', gap: 12 },
  memorySpine: { width: 3, height: 54, borderRadius: 2 },
  ...
  action: { minHeight: 42, paddingHorizontal: 31, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', alignItems: 'center', gap: 6 },
