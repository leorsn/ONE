...
export function retrieveLocalOneItems(
  query: string,
  items: OneItem[],
  options: { limit?: number; now?: Date; recentWhenEmpty?: boolean } = {}
): RetrievalResult[] {
  ...
  if (!clean) {
    ...
    return [...items]
      ...
      .map((item, index) => ({
        item,
        score: Math.max(0.1, 4 - index * 0.15),
        matchedTerms: [],
        reasons: ['recent']
      }));
  }
  ...
  for (const result of searchOneItems(clean, items, { limit: Math.max(limit * 2, 24), now })) {
    combined.set(result.item.id, result);
  }
  ...
  for (const item of timeAwareCandidates(clean, items, now)) {
    ...
    combined.set(item.id, {
      item,
      score: (existing?.score ?? 0) + 11,
      matchedTerms: existing?.matchedTerms ?? [],
      reasons: Array.from(new Set([...(existing?.reasons ?? []), 'time-context']))
    });
  }

  return Array.from(combined.values())
    .sort((a, b) => b.score - a.score || itemTimestamp(b.item) - itemTimestamp(a.item))
    .slice(0, limit);
}

export async function retrieveOneItems(
  query: string,
  items: OneItem[],
  options: {
    limit?: number;
    now?: Date;
    semanticSearch?: (query: string) => Promise<SemanticMatch[]>;
  } = {}
  ...
  try {
    ...
    for (const match of semantic) {
      ...
      combined.set(item.id, {
        item,
        score: (existing?.score ?? 0) + Math.max(0, match.similarity) * 12,
        matchedTerms: existing?.matchedTerms ?? [],
        reasons: Array.from(new Set([...(existing?.reasons ?? []), 'semantic'])),
    ...
    return {
      results: local,
      mode: 'local',
      semanticError: error instanceof Error ? error.message : 'semantic_search_unavailable'
    };
  }
}
...
function timeAwareCandidates(query: string, items: OneItem[], now: Date) {
  ...
  if (/\b(today|heute)\b/.test(normalized)) {
    return items.filter((item) => {
      const captured = (item.capturedAt || item.createdAt).slice(0, 10);
      return asksCaptured ? captured === today : captured === today || item.date === today;
    });
  }

  if (/\b(yesterday|gestern)\b/.test(normalized)) {
    return items.filter((item) => (item.capturedAt || item.createdAt).slice(0, 10) === yesterday);
  }
...
function itemTimestamp(item: OneItem) {
  return new Date(item.capturedAt || item.updatedAt || item.createdAt).getTime();
}
